...
HEROKU_PREVIEW="demo-pedro-preview"
HEROKU_PREPRODUCTION="demo-pedro-preproduction"
HEROKU_PRODUCTION="demo-pedro"
...
podTemplate(
    ...
    node ('test-pod') {
      container ('mvn') {
       ...
       echo env.HEROKU_PREVIEW
       echo env.HEROKU_PREPRODUCTION
...
def preview() {
  ...
  echo "Preview herokuApp: ${herokuApp}"
...
def preProduction() {
 ...
 echo "Pre Production herokuApp: ${herokuApp}"
...
def production() {
  ...
  echo "Production herokuApp: ${herokuApp}"
...
def herokuDeploy (herokuApp) {
 echo "herokuApp: ${herokuApp}"
    withCredentials([[$class: 'StringBinding', credentialsId: 'HEROKU_API_KEY_PEDROLACERDA', variable: 'HEROKU_API_KEY']]) {
...
def getCurrentHerokuReleaseVersion(app) {
    withCredentials([[$class: 'StringBinding', credentialsId: 'HEROKU_API_KEY_PEDROLACERDA', variable: 'HEROKU_API_KEY']]) {
        ...
        def response = sh(returnStdout: true, script: "curl -s  -H \"Authorization: Bearer ${env.HEROKU_API_KEY_PEDROLACERDA}\" -H \"Accept: application/vnd.heroku+json; version=3\" -X GET ${apiUrl}").trim()
        def jsonSlurper = new JsonSlurper()
        def data = jsonSlurper.parseText("${response}")
